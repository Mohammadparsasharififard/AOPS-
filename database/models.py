...
class SiteNode(Base):
    ...
    page_id: Mapped[Optional[str]] = mapped_column(ForeignKey("page.id"), nullable=True, index=True)
    asset_id: Mapped[Optional[str]] = mapped_column(ForeignKey("asset.id"), nullable=True, index=True)
    contest_id: Mapped[Optional[str]] = mapped_column(ForeignKey("contest.id"), nullable=True, index=True)
    contest_year_id: Mapped[Optional[str]] = mapped_column(ForeignKey("contest_year.id"), nullable=True, index=True)
    problem_id: Mapped[Optional[str]] = mapped_column(ForeignKey("problem.id"), nullable=True, index=True)
    problem_set_id: Mapped[Optional[str]] = mapped_column(ForeignKey("problem_set.id"), nullable=True, index=True)
    discussion_id: Mapped[Optional[str]] = mapped_column(ForeignKey("discussion.id"), nullable=True, index=True)
    post_id: Mapped[Optional[str]] = mapped_column(ForeignKey("post.id"), nullable=True, index=True)

    # Status — distinguishes ARCHIVED from BLOCKED/FAILED/SKIPPED/NOT_VERIFIED
    status: Mapped[str] = mapped_column(String(20), default="not_verified", index=True)
    # not_verified | discovered | archived | unchanged | failed | blocked | skipped

    # Metadata
    first_seen: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, index=True)
...
class SiteEdge(Base):
    ...
    source_node_id: Mapped[str] = mapped_column(
        ForeignKey("site_node.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class LocalUrlMapping(Base):
    ...
    canonical_source_url: Mapped[str] = mapped_column(Text, nullable=False, index=True)
    local_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # /archive/...
    node_id: Mapped[Optional[str]] = mapped_column(ForeignKey("site_node.id"), nullable=True, index=True)
