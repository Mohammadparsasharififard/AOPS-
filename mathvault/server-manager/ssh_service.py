...
# Categories: MathVault, System, Docker, Network, Security, Backup
SAFE_ACTIONS = [
    # --- MathVault Management ---
    {
        "id": "update-mathvault",
    ...
    {
        "id": "view-logs-100",
        "label": "View MathVault Logs (100 lines)",
        "description": "Tail last 100 log lines from MathVault API",
        "command": "sudo journalctl -u mathvault-api -n 100 --no-pager || sudo journalctl -u mathvault -n 100 --no-pager",
        "danger": False,
        "category": "MathVault",
    },
    {
        "id": "run-sync",
        "label": "Run Sync Now",
        "description": "Trigger an immediate incremental sync",
    ...
    },
    {
        "id": "dry-run",
        "label": "Dry-Run Crawl",
        "description": "Show what URLs the crawler WOULD fetch",
    ...
    },
    {
        ...
        "description": "Show DB connectivity + table counts",
    ...
    },
    {
        ...
        "description": "Show archive coverage (discovered/archived/blocked/failed)",
        "command": "cd /opt/mathvault && ./.venv/bin/python -m cli.mathvault coverage",
        "danger": False,
        "category": "MathVault",
    },
    {
        ...
        "description": "Check for broken links, missing assets, unindexed pages",
        "command": "cd /opt/mathvault && ./.venv/bin/python -m cli.mathvault validate-archive",
        "danger": False,
        "category": "MathVault",
    },
    {
        "id": "offline-test",
        "label": "Offline Test",
        "description": "Verify archive works without internet (0 external requests)",
        "command": "cd /opt/mathvault && ./.venv/bin/python -m cli.mathvault offline-test",
        "danger": False,
        "category": "MathVault",
    },
    {
        "id": "retry-blocked",
        "label": "Retry Blocked URLs",
        "description": "Re-fetch URLs that were blocked (CAPTCHA, login, etc.)",
        "command": "cd /opt/mathvault && ./.venv/bin/python -m cli.mathvault retry-blocked",
        "danger": False,
        "category": "MathVault",
    },
    {
    ...
    },
    {
        "id": "archive-size",
        "label": "Archive Size",
        "description": "Show total size of archive directory + breakdown",
        "command": "du -sh /opt/mathvault/archive/ 2>/dev/null && du -sh /opt/mathvault/archive/pages/ /opt/mathvault/archive/assets/ 2>/dev/null || true",
        "danger": False,
        "category": "MathVault",
    },
    {
        "id": "db-size",
        "label": "Database Size",
        "description": "Show database file size + row counts per table",
        "command": "ls -lh /opt/mathvault/data/*.db 2>/dev/null; cd /opt/mathvault && ./.venv/bin/python -c \"import sqlite3; db=sqlite3.connect('data/mathvault.db'); [print(f'{r[0]}: {r[1]} rows') for r in db.execute('SELECT name, (SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=m.name) FROM sqlite_master m WHERE type=\\\"table\\\"').fetchall()]\" 2>/dev/null || true",
        "danger": False,
        "category": "MathVault",
    },
    ...
    # --- System Diagnostics ---
    {
        "id": "system-info",
        "label": "System Info",
        "description": "OS version, kernel, architecture, hostname",
        "command": "hostnamectl 2>/dev/null || uname -a",
        "danger": False,
        "category": "System",
    },
    {
        "id": "cpu-info",
        "label": "CPU Info",
        "description": "CPU cores, model, frequency, load average",
        "command": "echo '=== CPU ===' && lscpu | grep -E 'Model name|CPU\\(s\\)|Thread|Core|Socket|MHz' && echo '=== Load ===' && cat /proc/loadavg && echo '=== Top CPU ===' && ps aux --sort=-%cpu | head -10",
        "danger": False,
        "category": "System",
    },
    {
        "id": "memory-info",
        "label": "Memory Info",
        "description": "RAM + swap usage, detailed breakdown",
        "command": "echo '=== Memory ===' && free -h && echo '=== Swap ===' && swapon --show 2>/dev/null || true && echo '=== Top Memory ===' && ps aux --sort=-%mem | head -10",
        "danger": False,
        "category": "System",
    },
    {
        "id": "disk-usage",
        "label": "Disk Usage (all mounts)",
        "description": "Show disk usage for all mounted filesystems",
        "command": "df -h --type=ext4 --type=xfs --type=trfs --type=btrfs 2>/dev/null || df -h",
        "danger": False,
        "category": "System",
    },
    {
        "id": "disk-top",
        "label": "Top Directories by Size",
        "description": "Show largest directories on /opt",
    ...
    {
        "id": "uptime",
        "label": "Uptime + Boot Info",
        "description": "System uptime, last boot, boot ID",
        "command": "uptime && echo '---' && who -b 2>/dev/null || true",
        "danger": False,
        "category": "System",
    },
    {
        "id": "processes-top",
        "label": "Top Processes (CPU+Memory)",
    ...
    },
    {
        ...
        "description": "Show all systemd services (running + failed)",
        "command": "systemctl list-units --type=service --state=running --no-pager --no-legend | head -50; echo '=== Failed ==='; systemctl --failed --no-pager --no-legend",
        "danger": False,
        "category": "System",
    },
    {
        "id": "cron-list",
        "label": "Cron Jobs",
        "description": "Show all cron jobs for root + current user",
        "command": "echo '=== Root crontab ===' && sudo crontab -l 2>/dev/null || true && echo '=== User crontab ===' && crontab -l 2>/dev/null || true && echo '=== /etc/cron.d ===' && ls -la /etc/cron.d/ 2>/dev/null || true",
        "danger": False,
        "category": "System",
    },
    {
        ...
        "description": "Show all systemd timers (next run time)",
    ...
    {
        "id": "docker-psa",
        "label": "Docker: All Containers (incl. stopped)",
    ...
    },
    {
        ...
        "description": "Show all Docker images + sizes",
    ...
    {
        "id": "docker-volumes",
        "label": "Docker: List Volumes",
        "description": "Show all Docker volumes + sizes",
        "command": "docker volume ls && echo '---' && docker system df",
        "danger": False,
        "category": "Docker",
    },
    {
        "id": "docker-logs",
        "label": "Docker: MathVault Logs",
        "description": "Show last 50 lines of MathVault Docker container logs",
        "command": "docker logs mathvault-api --tail 50 2>&1 || docker logs mathvault --tail 50 2>&1 || echo 'Container not found'",
        "danger": False,
        "category": "Docker",
    },
    {
        ...
        "description": "Show CPU/memory/network I/O per container (live snapshot)",
        "command": "timeout 5 docker stats --no-stream --format 'table {{.Name}}\\t{{.CPUPerc}}\\t{{.MemUsage}}\\t{{.NetIO}}\\t{{.BlockIO}}'",
        "danger": False,
        "category": "Docker",
    },
    {
        ...
        "command": "cd /opt/mathvault && docker compose ps 2>/dev/null || docker-compose ps 2>/dev/null || echo 'No compose project'",
        "danger": False,
        "category": "Docker",
    },
    ...
    # --- Network Diagnostics ---
    {
        ...
        "description": "Show all network interfaces + IP addresses",
        "command": "ip -o addr show scope global | awk '{print $2, $3, $4}' | head -20",
        "danger": False,
        "category": "Network",
    },
    {
        ...
        "description": "Show all listening ports + processes",
    ...
    },
    {
        ...
        "label": "Network: Active Connections",
        "description": "Show active network connections (established)",
        "command": "ss -tnp state established | head -30",
        "danger": False,
        "category": "Network",
    },
    {
        "id": "network-dns",
        "label": "Network: DNS Config",
        "description": "Show DNS resolver configuration",
        "command": "cat /etc/resolv.conf && echo '---' && cat /etc/hosts",
        "danger": False,
        "category": "Network",
    },
    {
        "id": "network-firewall",
        "label": "Network: Firewall Status",
        "description": "Show firewall rules (ufw or iptables)",
        "command": "sudo ufw status verbose 2>/dev/null || sudo iptables -L -n --line-numbers 2>/dev/null || echo 'No firewall found'",
        "danger": False,
        "category": "Network",
    },
    {
        "id": "network-route",
        "label": "Network: Routing Table",
    ...
    # --- Security Checks ---
    {
        ...
        "label": "Security: Failed SSH Logins",
        "description": "Show recent failed SSH login attempts (last 20)",
        "command": "sudo journalctl -u ssh -u sshd --no-pager -n 200 2>/dev/null | grep -i 'failed\\|invalid' | tail -20 || echo 'No failed logins found'",
        "danger": False,
        "category": "Security",
    },
    {
        "id": "security-last-logins",
        "label": "Security: Last Logins",
        "description": "Show last 20 successful logins",
        "command": "last -20 | head -20",
        "danger": False,
        "category": "Security",
    },
    {
        "id": "security-users",
        "label": "Security: Users with Shell",
        "description": "Show all users with login shell",
        "command": "grep -E '/bin/(bash|zsh|sh|fish)$' /etc/passwd",
        "danger": False,
        "category": "Security",
    },
    {
        "id": "security-ssh-keys",
        "label": "Security: SSH Authorized Keys",
        "description": "Show authorized SSH keys for current user",
        "command": "cat ~/.ssh/authorized_keys 2>/dev/null | wc -l && echo 'keys found' || echo 'No authorized_keys file'",
        "danger": False,
        "category": "Security",
    },
    {
        "id": "security-updates",
        "label": "Security: Available Updates",
        "description": "Check for available system updates (apt)",
        "command": "apt list --upgradable 2>/dev/null | head -30 || echo 'apt not available'",
        "danger": False,
        "category": "Security",
    },
    {
        "id": "security-reboot-needed",
        "label": "Security: Reboot Required?",
        "description": "Check if system needs a reboot (after kernel update)",
        "command": "if [ -f /var/run/reboot-required ]; then echo 'REBOOT REQUIRED'; else echo 'No reboot needed'; fi",
        "danger": False,
        "category": "Security",
    },
    ...
    # --- Backup Management ---
    {
        ...
        "description": "Show all backup files with sizes + dates",
        "command": "ls -lht /opt/mathvault/backups/*.tar.gz 2>/dev/null | head -20 || echo 'No backups found'",
        "danger": False,
        "category": "Backup",
    },
    {
        "id": "backup-now",
        "label": "Backup: Create Now",
        "description": "Create a manual backup (DB + archive) → /opt/mathvault/backups/",
    ...
    {
        "id": "backup-cleanup",
        "label": "Backup: Clean Old (30+ days)",
        "description": "Delete backups older than 30 days",
        "command": "find /opt/mathvault/backups/ -name '*.tar.gz' -mtime +30 -delete 2>/dev/null && echo 'Old backups cleaned' || echo 'No backups to clean'",
        "danger": True,
        "category": "Backup",
    },
    {
        "id": "backup-verify",
        "label": "Backup: Verify Latest",
        "description": "Verify SHA-256 of the latest backup file",
        "command": "ls -t /opt/mathvault/backups/*.tar.gz 2>/dev/null | head -1 | xargs -I{} sh -c 'echo \"File: {}\" && sha256sum {} && if [ -f {}.sha256 ]; then echo \"Expected:\" && cat {}.sha256 && echo && sha256sum -c {}.sha256; else echo \"No .sha256 file\"; fi' || echo 'No backups found'",
        "danger": False,
        "category": "Backup",
    },
]
